...
/**
 * Where the ruleset may live, in priority order:
 * 1. Bundled next to the extension (extensions/skills/i-have-adhd).
 * 2. Pi's global skill directory (~/.pi/agent/skills).
 * 3. The cross-harness skill directory (~/.agents/skills), where skill
 *    managers install by default.
 */
function skillCandidatePaths(): string[] {
  return [
    join(EXTENSION_DIR, "..", "skills", "i-have-adhd", "SKILL.md"),
    join(getAgentDir(), "skills", "i-have-adhd", "SKILL.md"),
    join(homedir(), ".agents", "skills", "i-have-adhd", "SKILL.md"),
  ];
}
...
function loadRules(): string {
  const candidates = skillCandidatePaths();
  const skillPath = candidates.find((path) => existsSync(path));

  if (!skillPath) {
    throw new Error(
      `Unable to locate i-have-adhd SKILL.md. Tried:\n${candidates
        .map((path) => `  - ${path}`)
        .join("\n")}`,
    );
  }
  ...
  try {
    ...
    throw new Error(
      `Unable to load i-have-adhd rules from ${skillPath}: ${reason}`,
    );
  }

  const rules = stripFrontmatter(content);
  if (!rules) {
    throw new Error(`The i-have-adhd rules file is empty: ${skillPath}`);
  }
